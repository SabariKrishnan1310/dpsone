...
class Command(BaseCommand):
    help = 'Run the F&B worker to process tap events from Redis streams'

    def handle(self, *args, **kwargs):
        logger.error("F&B Worker started")
                ...
                for stream_name, messages in result:
                    for message_id, fields in messages:
                        try:
                            ...
                            logger.info("Processing F&B tap: %s", data)
                            
                            success = worker.process_fnb_tap(data)
